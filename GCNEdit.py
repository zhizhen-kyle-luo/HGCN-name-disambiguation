...
class GCN(GraRep):
    ...
    def _optimize_line(self):
        ...
        self.u_i = tf.keras.Input(name='u_i', shape=(self.sample_num,), dtype=tf.int32)
        self.u_j = tf.keras.Input(name='u_j', shape=(self.sample_num,), dtype=tf.int32)
        self.label = tf.keras.Input(name='label', shape=(self.sample_num,), dtype=tf.float64)
        ...
        # Define the operation as a Keras layer
        self.u_i_embedding = tf.keras.layers.Lambda(
            lambda x: tf.matmul(tf.one_hot(x, depth=self.node_num, dtype=tf.float64), self.embed),
            output_shape=(self.sample_num, self.embed.shape[1])
        )(self.u_i)
        
        self.u_j_embedding = tf.keras.layers.Lambda(
            lambda x: tf.matmul(tf.one_hot(x, depth=self.node_num, dtype=tf.float64), self.embed),
            output_shape=(self.sample_num, self.embed.shape[1])
        )(self.u_j)
        
        # self.inner_product = tf.reduce_sum(self.u_i_embedding * self.u_j_embedding, axis=1)
        # New Lambda layer for inner product
        self.inner_product = tf.keras.layers.Lambda(
            lambda x: tf.reduce_sum(x[0] * x[1], axis=1),
            output_shape=(self.sample_num,)
        )([self.u_i_embedding, self.u_j_embedding])
        
        # reg = tf.contrib.layers.apply_regularization(tf.contrib.layers.l2_regularizer(1e-4), tf.trainable_variables())
        # Define the regularization term using tf.keras.regularizers
        l2_regularizer = tf.keras.regularizers.L2(1e-4)
        reg = tf.add_n([l2_regularizer(var) for var in self.trainable_variables])
